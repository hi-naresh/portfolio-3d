...
const Header: React.FC = React.memo(() => (
	<motion.div
		...
		initial={{ scaleX: 0, opacity: 0 }} // Expand from center
		animate={{ scaleX: 1, opacity: 1 }}
		transition={{ delay: 0.2, duration: 1.5, ease: "easeInOut" }}
		style={{ transformOrigin: "center" }} // Scale from center
	>
		<motion.div
			className="glassmorphism  w-full max-w-[60rem] sm:max-w-[40rem] px-1.5 md:py-[0.3rem] xs:py-1.5 rounded-full flex justify-between items-center space-x-4 bg-white bg-opacity-10 border border-white/20" // Reapplying glassmorphism effects
        ...
        <span>
          <img className="w-3 h-3 mr-2 filter invert" src="/assets/icons/lock.svg" alt={"secure"} />
			...
			>
				<motion.button
					className="glassBg bg-white bg-opacity-10 xs:hidden md:flex rounded-full p-2 " // Applying backdrop blur for glassmorphism
